...
class MediaImport
{
    ...
    public function __construct($pdf_uploaded_file = "", $job_number = 110011)
    {
        ...
        $val = $this->db_exp->table("media_job")->where('pdf_file', $pdf_filename)->select('job_id');
        foreach ($val as $u) {
        ...
        if ($this->job_id == '') {
            $this->db_exp->table("media_job")->insert([
                'job_number' => $job_number,
                'pdf_file' => $pdf_filename,
            ]);
            $this->job_id = $this->db_exp->getInsertId();
        }
        ...
        $pdfObj = new PDFImport($pdf_uploaded_file, $this->job_id);
        $pdf = $pdfObj->form;
        if (count($pdf) < 1) {
            return 0;
        }
        ...
        $this->db_exp->table('media_job')->where('job_id', $this->job_id)->update(['close' => $pdf[$keyidx]['details']['product']]);
    ...
    {
        $this->db_exp->table("media_forms")->insert($form_array);
    }
    ...
    public function add_form_data($form_number, $form_array)
    {
        ...
        foreach ($forms as $letter => $row) {
            foreach ($row as $individual_part) {
                ...
                $this->db_exp->table("form_data")->insert($individual_part);
            }
        }
    }
}
